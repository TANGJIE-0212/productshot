...
(async () => {
  try {
    ...
    publish("selection", { capabilityIds: [], additional: "", scope: "whole", audience: "" });
    publish("selection", { capabilityIds: [], additional: "Custom capability", scope: "related", audience: "" });
    ...
    assert.match(await page.$eval("main", (e) => e.textContent), /在当前 Agent 对话中运行 Skill/);
    ...
    await page.click('[data-action="refresh"]');
    await page.waitForFunction(() => document.querySelector('[data-path="notes"]').value === "Hold the result longer.");
    publish("selection", { ...selection, audience: "" });
    ...
    assert.equal(read().documents.selection.audience, "");
    await page.click('[data-scope-mode="details"]');
    await page.click('[data-capability="form"]');
    await page.type('[data-path="additional"]', "My own new feature");
    await page.click('[data-action="save"]');
    await page.waitForFunction(() => document.getElementById("notice").textContent.includes("展示内容已保存"));
